...
int main()
{
 ...
 // Произвольный размер буфера использовать нельзя - mmap не разрешает задавать offset, не кратный размеру страницы. Одну страницу отображать неудобно, потому что размер заносимой в файл структуры не является делителем размера страницы и вообще не является постоянным (как минимум имя файла может быть разной длины). Дробить на байты переменную типа int на границе страниц файла не хочется. Поэтому отображаю две страницы, а когда подхожу к концу второй, "выкидываю" первую (полностью заполненную) и загружаю еще одну (см. комментарий к функции remap в mmap_writer.c).

//Единственный недостаток - я использую то, что размер структуры с информацией о файле меньше размера страницы. Но если бы это было не так, то можно было бы отобразить не по две страницы, а по 4, и применить тот же алгоритм.
	...
	// Написала 777, не подумав((
	// исправила (в mmap_writer тоже):
