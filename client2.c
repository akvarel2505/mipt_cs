...
int main()
{
	...
	//Разделяемая память нужна для того, чтобы клиенты получили свои номера. Самый первый клиент, создающий семафор, присваивает себе id=1 и оставляет в разделяемой памяти id следующего клиента. Id связан с порядковым номером (1,2,3,4...) так: 4*(num-1)+1;
	(void)umask(0);
